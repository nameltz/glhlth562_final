...
debtspending %>%
  group_by(year, state) %>%
  summarise(avg_chg = mean(loan_pct_chg), na.rm = TRUE) %>%
  ungroup() %>%
  group_by(year) %>%
  summarise(avg_loan_chg = mean(avg_chg), na.rm = TRUE)%>%
  ungroup()%>%
  mutate(year = as.numeric(year))%>%
ggplot(aes(x = year, 
            y = avg_loan_chg))+
  ...
  theme(plot.title.position = "plot",
        ...
        plot.subtitle = element_text(size=15),
        plot.caption = element_text(size=10))+
  labs(title = "Annual change in student loans per capita",
       caption = "Data: Federal Reserve Bank of New York, Quarterly Report on Household Debt and Credit",
       x = "",
       y = "")
...
debtspending %>%
  ...
  scale_fill_gradient(low = "white", 
                        high = "red",
                        name = "Avg. student loans per capita, 2004",
